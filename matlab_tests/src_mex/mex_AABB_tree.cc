...
#define CMD MEX_ERROR_MESSAGE_9
    ...
    ptr->get_bbox_indexes_of_a_node( inode - 1,
                                     bb_index );  // C/C++ 0-based indexing
...
#define CMD MEX_ERROR_MESSAGE_12
    ...
    UTILS_MEX_ASSERT( dim0 == dim1, "{}: size(bb_min) = {} must be equal to size(bb_max) = {}\n", dim0, dim1 );
...
#define CMD MEX_ERROR_MESSAGE_17
    ...
    int64_t     nmin = Utils::mex_get_int64( arg_in_2, CMD ": parameter nmin" );
    if ( nmin == 0 )
      Utils::mex_set_scalar_int32( arg_out_0, ptr->num_tree_nodes() );
    else
      Utils::mex_set_scalar_int32( arg_out_0, ptr->num_tree_nodes( nmin ) );
#undef CMD
  }
