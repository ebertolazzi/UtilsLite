...
public:
  ...
  exact_solution( vector<Vector> & x_vec ) const override {
    ...
    auto & x0{ x_vec[0] };
    x0.resize(n);
    x0 << 1.000000000000000000000000000000000000000,
          18.71828182845904523536028747135266249776,
          2.028653803092486959006105196343825624600,
          279.8410647514915106118923581352729397598,
          6.876553786347144986146293257311354434906,
          976.1568043532779061610180406939220907971,
          -61079.62412297820687494549945165294788674,
          986.0620119427175381061704391475772708566;
  }
...
public:
  ...
  jacobian( Vector const & x, SparseMatrix & J ) const override {
    ...
    J.insert(18,17) = 0.5*x10/sqrt(x10*x16);
    J.insert(18,18) = -1;
  ...
  initial_points( vector<Vector> & x_vec ) const override {
    ...
    auto & x0{ x_vec[0] };
    x0.resize(n);
    x0 << 1.5, 1, 50, 50, 500, 5000, 0, 5000, 0, 5000, 0.5,
          50, 5, 0.5, 50, 50, 5, 50, 50, 5, 50, 500, 5, 50,
          -50, 5000, 0.05, 50000, 5, 5000;
  }
