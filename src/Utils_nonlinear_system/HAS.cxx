...
public:
  ...
  jacobian( Vector const & x, SparseMatrix & J ) const override {
    ...
    J_full(0,0) += 100000/(x0*x0x0);
    J_full(1,1) += 144000/(x1*x1x1);
    J_full(2,2) += 288000/(x2*x2x2);
    ...
    J_full(4,0) -= 4*x4*tau;
    J_full(4,4) += 4*(3*x4x4-x0+0.00001)*tau;

    J_full(5,1) -= 4*x5*tau;
    J_full(5,5) += 4*(3*x5x5-x1+0.00001)*tau;

    J_full(6,2) -= 4*x6*tau;
    J_full(6,6) += 4*(3*x6x6-x2+0.00001)*tau;
    ...
    J_full(0,0) += tt1*(tmp_0-2*tmp/x0);
    J_full(0,1) += tt1*tmp_1;
    J_full(0,2) += tt1*tmp_2;
    J_full(0,3) += tt1*tmp_3;

    real_type tt2 = 64*tau/x1x1;
    J_full(1,0) += tt2*tmp_0;
    J_full(1,1) += tt2*(tmp_1-2*tmp/x1);
    J_full(1,2) += tt2*tmp_2;
    J_full(1,3) += tt2*tmp_3;

    real_type tt3 = 240*tau/x2x2;
    J_full(2,0) += tt3*tmp_0;
    J_full(2,1) += tt3*tmp_1;
    J_full(2,2) += tt3*(tmp_2-2*tmp/x2);
    J_full(2,3) += tt3*tmp_3;

    real_type tt4 = 4*tau;
    J_full(3,0) -= tt4*(tmp_0*x3);
    J_full(3,1) -= tt4*(tmp_1*x3);
    J_full(3,2) -= tt4*(tmp_2*x3);
    J_full(3,3) -= tt4*(tmp-2*x3x3);
  ...
  exact_solution( vector<Vector> & x_vec ) const override {
    ...
    x0.resize(n);
    if ( tau == 1e2 ) {
      x0 << 6.6818141799970216926, 9.0745104956268746744, 12.995355396076109065, 0, 0, 0, 0;
    } if ( tau == 1e4 ) {
      x0 << 3.9001579154194764056, 7.7092485438427308755, 11.962907630149328334, 0, 0, 0, 0;
    } if ( tau == 1e6 ) {
      x0 << 3.847300847063338, 7.694589110936490, 11.954435422989896, 0, 0, 0, 0;
    } if ( tau == 1e8 ) {
      x0 << 3.8473008470633382174, 7.6945891109364897632, 11.954435422989895821, 0, 0, 0, 0;
    } if ( tau == 1e10 ) {
      x0 << 3.8472955440686861651, 7.6945876623010364843, 11.954434592374304256, 0, 0, 0, 0;
    }
  }
...
inline
string
ini_msg_HAS93( real_type tau ) { return fmt::format( "HAS 93, tau = {}", tau ); }
...
public:
  ...
  jacobian( Vector const & x, SparseMatrix & J ) const override {
    ...
    J_full.setZero();
    {
      ...
      J_full(0,0)   = 0.2E-5*t3*t8+2.0*t22+((0.15376E-5*t24*x(0)+0.14384E-5*t28)*x(1)+0.15376E-5*t24*t32+0.15376E-5*t24*x(2)*x(0))*t4+0.248E-2*(-1.0+0.58E-3*t18*t40+0.9106E-3*t1*x(2)*t7+t46)*t5*x(3)+2.0;
      J_full(0,1)   = t89;
      J_full(0,2)   = t107;
      J_full(0,3)   = t127;
      ...
      J_full(1,0)   = t89;
      J_full(1,1)   = 0.2E-5*t32*t2*t8+2.0*t180+(0.331676944E-5*t1*t182+(0.2112592E-5*t182*x(0)+0.2112592E-5*t182*x(3))*x(1)+0.2258288E-5*t40*t11)*t2+0.36424E-2*(-1.0+0.62E-3*x(3)*t32*t5+0.62E-3*t15*t5*x(1)+t46)*t7*x(2)+2.0;
      J_full(1,2)   = t220;
      J_full(1,3)   = t231;
      J_full(1,4)   = t242;
      ...
      J_full(4,2)   = t301;
      J_full(4,3)   = t337;
      J_full(4,4)   = 0.2E-5*t273*t356*t7+0.30752E-5*t32*t4*t361*t5-0.248E-2*t239*x(3)*t12+2.0;
      J_full(4,5)   = t380;
      J_full(4,6)   = -t382;
      J_full(4,7)   = t385;
      J_full(4,12)  = -2.0;
...

    {
      ...
      J_full(0,1) += t12;
      J_full(0,2) += t16;
      J_full(0,3) += (0.1214*x(0)+0.607E-1*x(1)+0.607E-1*x(2))*t1+0.408E-1*x(0)+0.204E-1*x(1)+0.204E-1*x(2);
      J_full(0,4) += t31;
      J_full(0,5) += t34;
      J_full(1,0) += t12;
      J_full(1,1) += 0.137218*t10+0.58718E-1*x(2);
      J_full(1,2) += (0.437E-1*x(0)+0.137218*x(1)+0.437E-1*x(3))*t9+0.187E-1*x(0)+0.58718E-1*x(1)+0.187E-1*x(3);
      J_full(1,3) += t50;
      J_full(1,4) += t53;
      J_full(1,5) += t59;
      J_full(2,0) += t16;
...
public:
  ...
  jacobian( Vector const & x, SparseMatrix & J ) const override {
    ...
    J_full(2,2) += fact*( 2.0*ex2*(2.0*ex0+4.0*ex1+10.0*ex2+2.0*ex5+3.0*ex9-5.0+ex6+ex7+2.0*ex8) );
    ...
    J_full(5,5) += fact*( 2.0*ex5*(ex0+2.0*ex1+2.0*ex2+4.0*ex5+ex9-3.0+ex3+2.0*ex4+ex6) );
    ...
    J_full(9,9) += fact*( 2.0*ex9*(ex0+2.0*ex1+3.0*ex2+ex5+4.0*ex9-3.0+ex6+ex7+2.0*ex8) );
