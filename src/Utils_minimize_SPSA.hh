...
#ifndef UTILS_MINIMIZE_SPSA_dot_HH
#define UTILS_MINIMIZE_SPSA_dot_HH
...
namespace Utils
{
   ...
   * @tparam Scalar Data type (double, float, etc.)
   */
  template <typename Scalar = double> class SPSA_minimizer
  {
  public:
    using Vector   = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;  ///< Vector type
    using Callback = std::function<Scalar( Vector const & )>;   ///< Function callback type
  ...
  private:
     ...
     * @param f Current function value
     * @param previous_f Previous function value
     * @param gnorm Current gradient norm
     * @param total_consecutive_grad Counter for gradient norm criterion
     * @param total_consecutive_func Counter for function change criterion
     * @param no_improvement_count Counter for stagnation
     * @param[out] converged True if converged
     * @param[out] message Convergence message
    ...
    bool check_convergence(
      ...
      Scalar   previous_f,
      Scalar   gnorm,
      size_t & total_consecutive_grad,
      size_t & total_consecutive_func,
      size_t   no_improvement_count,
      bool &   converged,
      string & message ) const
    {
      ...
      {
        converged = true;
        message   = "No improvement for patience iterations";
        return true;
      }
  ...
  public:
     ...
     * @return True if minimization converged successfully
     */
    bool minimize( Vector const & x0, Callback const & fun )
    {
      ...
      for ( size_t k = 0; k < m_opts.max_iter; ++k )
      {
        ...
        for ( size_t rep = 0; rep < m_opts.gradient_avg; ++rep )
        {
          ...
          // Note: (xp - xm) = 2 * c_k * delta, but we use direct division for numerical stability
          g.array() += ( fp - fm ) / ( 2 * c_k * delta.array() );
        }
        ...
        // Verbose output (every print_every iterations to avoid clutter)
            ...
            convergence_message ) )
        {
          ...
          m_final_x      = best_x;
          m_converged    = true;
          m_f_eval_count = f_eval_count;
          m_message      = convergence_message;
      ...
      // Store final results (maximum iterations reached)
      m_final_f      = best_f;
      m_grad_norm    = 0;  // Not computed at final point
      m_iterations   = m_opts.max_iter;
      m_final_x      = best_x;
      m_converged    = false;
      m_f_eval_count = f_eval_count;
      m_message      = "Maximum iterations reached";
     ...
     */
    Scalar grad_norm() const { return m_grad_norm; }
    ...
    /**
     * @brief Get final solution vector
     * @return Final solution
     */
    Vector const & final_x() const { return m_final_x; }
     ...
     */
    size_t f_eval_count() const { return m_f_eval_count; }

    /**
     * @brief Get termination message
