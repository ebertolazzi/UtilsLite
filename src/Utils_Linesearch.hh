...
namespace Utils
{
  ...
  namespace LS_detail
  {
     ...
     */
    template <class Scalar> inline Scalar cubic_minimizer(
      Scalar a,
      Scalar fa,
      Scalar fpa,
      Scalar b,
     ...
     */
    template <typename Scalar> Scalar compute_step(
      Scalar                  a_lo,
      Scalar                  phi_lo,
      Scalar                  der_lo,
      Scalar                  a_hi,
   ...
   */
  template <typename Scalar> class ArmijoLineSearch
  {
  ...
  public:
     ...
     */
    template <typename Callback> std::optional<std::tuple<Scalar, size_t>> operator()(
      Scalar                   f0,
      Scalar                   Df0,
      Eigen::Ref<Vector const> x,
      Eigen::Ref<Vector const> d,
      Callback const &         callback,
      Scalar                   step0 = 1 ) const
    {
      Scalar     step   = step0;
  ...
  template <typename Scalar> class WeakWolfeLineSearch
  {
    ...
    template <typename EvalFunc> Scalar zoom(
      Scalar           a_lo,
      Scalar           f_lo,
      Scalar           Df_lo,
      Scalar           a_hi,
   ...
   */
  template <typename Scalar> class StrongWolfeLineSearch
  {
    using Vector = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
      ...
      Scalar                   alpha0 = 1 ) const
    {
      ...
      while ( n_evals < m_max_iters )
      {
        Scalar a_j = abs( alpha_hi - alpha_lo ) > m_epsi
                       ? LS_detail::cubic_minimizer<Scalar>( alpha_lo, phi_lo, der_lo, alpha_hi, phi_hi, der_hi )
                       : ( alpha_lo + alpha_hi ) / 2;
   ...
   */
  template <typename Scalar> class GoldsteinLineSearch
  {
    using Vector         = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
   ...
   */
  template <typename Scalar> class HagerZhangLineSearch
  {
  public:
    using Vector = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
  ...
  private:
     ...
     *
     * Uses cubic minimization (LS_detail::cubic_minimizer) then enforces safety:
  ...
  template <typename Scalar> class MoreThuenteLineSearch
  {
    using Vector = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
      ...
      Scalar                   alpha0 = 1 ) const
    {
      ...
      for ( size_t k = 0; k < m_max_iters; ++k )
      {
        ...
        if ( bracketed )
        {
          ...
          // alpha_hi
          alpha_new = LS_detail::compute_step(
            alpha_lo,
            phi_lo,
            der_lo,
            alpha_hi,
        ...
        else
        {
          ...
          // rischiosa) alpha_new = LS_detail::compute_step(
