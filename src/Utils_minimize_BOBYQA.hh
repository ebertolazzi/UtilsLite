...
namespace Utils
{
  ...
  */
  ...
  template <typename Scalar> class BOBYQA_minimizer
  {
  public:
    ...
    static string to_string( Status status )
    {
      switch ( status )
      {
        case Status::BOBYQA_SUCCESS: return "algorithm converged";
        case Status::BOBYQA_BAD_NPT: return "NPT is not in the required interval";
        case Status::BOBYQA_TOO_CLOSE: return "insufficient space between the bounds";
        case Status::BOBYQA_ROUNDING_ERRORS: return "too much cancellation in a denominator";
        case Status::BOBYQA_TOO_MANY_EVALUATIONS: return "maximum number of function evaluations exceeded";
        case Status::BOBYQA_STEP_FAILED: return "a trust region step has failed to reduce Q";
      }
      return "";
    }
  ...
  private:
    ...
    static Scalar power2( Scalar const x ) { return x * x; }
    static bool   is_zero( Scalar const x ) { return std::abs( x ) < 1e-20; }

  public:
    void set_rho( Scalar beg, Scalar end )
    {
      m_rhobeg = beg;
      m_rhoend = end;
    }
    void set_verbosity( integer v ) { m_print_level = v; }
  ...
  public:
    ...
    void    debug_set_B( Matrix const & B ) { m_B = B; }
    void    debug_set_Z( Matrix const & Z ) { m_Z = Z; }
    void    debug_set_vlag( Vector const & vlag ) { m_v_lag = vlag; }
    void    debug_set_pq( Vector const & pq ) { m_pq = pq; }
    void    debug_set_knew( integer knew ) { m_knew = knew; }
    void    debug_set_denom( Scalar denom ) { m_denom = denom; }
    void    debug_set_beta( Scalar beta ) { m_beta = beta; }
    Matrix  debug_B() const { return m_B; }
    Matrix  debug_Z() const { return m_Z; }
    Vector  debug_vlag() const { return m_v_lag; }
    Vector  debug_pq() const { return m_pq; }
    integer debug_knew() const { return m_knew; }
    Scalar  debug_denom() const { return m_denom; }
    Scalar  debug_beta() const { return m_beta; }
    // Execute the private update() method for testing
    void debug_update() { update(); }
   ...
   */
  template <typename Scalar> typename BOBYQA_minimizer<Scalar>::Status BOBYQA_minimizer<Scalar>::minimize(
    integer const         N,
    integer const         NPT,
    bobyqa_objfun const & fun,
    Vector &              X,
  ...
  /*----------------------------------------------------------------------------*/
  template <typename Scalar> void BOBYQA_minimizer<Scalar>::prelim( Vector & X )
  {
  ...
  template <typename Scalar> void BOBYQA_minimizer<Scalar>::altmov()
  {
  ...
  template <typename Scalar> void BOBYQA_minimizer<Scalar>::rescue()
  {
    ...
    while ( nrem > 0 )
    {
      ...
      for ( integer k = 0; k < m_npt; ++k )
      {
        ...
        else
        {
          ...
          Scalar  iq_real = static_cast<Scalar>( np ) * m_ptsid( k ) -
                            static_cast<Scalar>( ip ) * static_cast<Scalar>( np );
    ...
    for ( integer kpt = 0; kpt < m_npt; ++kpt )
    {
      ...
      Scalar  iq_real = static_cast<Scalar>( np ) * m_ptsid( kpt ) -
                        static_cast<Scalar>( ip ) * static_cast<Scalar>( np );
      integer iq      = static_cast<integer>( iq_real );  // troncamento
   ...
   */
  template <typename Scalar> void BOBYQA_minimizer<Scalar>::trsbox()
  {
    // ============================================================================
   ...
   */
  template <typename Scalar> void BOBYQA_minimizer<Scalar>::update()
  {
