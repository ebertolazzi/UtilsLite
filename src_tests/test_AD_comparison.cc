...
namespace
{
  ...
  {
    return x * x + 3.0 * x * y + y * y + std::sin( x ) * std::exp( y ) + std::log( 2.0 + x * x + y * y ) +
           std::cos( x * y );
  }
  ...
  Derivatives tinyad_derivatives( double x, double y )
  {
    static auto function = []
    {
      ...
      result.template add_elements<1>(
        TinyAD::range( 1 ),
        []( auto & element )
        {
          ...
          return variables[0] * variables[0] + 3.0 * variables[0] * variables[1] + variables[1] * variables[1] +
                 sin( variables[0] ) * exp( variables[1] ) +
                 log( 2.0 + variables[0] * variables[0] + variables[1] * variables[1] ) +
...
int main()
{
  ...
  std::cout << std::setprecision( 16 ) << "Autodiff vs TinyAD: accuracy and timing comparison\n"
  ...
  std::cout << "[Timing] Running " << timing_rounds << " rounds x " << points.size()
            << " points = " << timing_evaluations << " value/gradient/Hessian evaluations per implementation...\n";
  const double analytical_ms = elapsed_milliseconds(
    []( double px, double py ) { return analytical_derivatives( px, py ); },
    points );
  const double autodiff_ms = elapsed_milliseconds(
    []( double px, double py ) { return autodiff_derivatives( px, py ); },
    points );
  const double tinyad_ms = elapsed_milliseconds(
    []( double px, double py ) { return tinyad_derivatives( px, py ); },
    points );
  const double tinyad_scalar_ms = elapsed_milliseconds(
    []( double px, double py ) { return tinyad_scalar_derivatives( px, py ); },
    points );
  ...
  std::cout
    ...
    << std::setw( 20 ) << 0.0 << " | " << std::setw( 20 ) << analytical_rms_error << " | " << std::fixed
    << std::setprecision( 6 ) << std::setw( 10 ) << analytical_ms << " ms | " << std::setw( 15 )
    << 1e3 * analytical_ms / timing_evaluations << " |\n"
    << "| autodiff    | " << std::setw( 5 ) << points.size() << " | " << std::scientific << std::setprecision( 3 )
    << std::setw( 20 ) << worst_autodiff_error << " | " << std::setw( 20 ) << autodiff_rms_error << " | " << std::fixed
    << std::setprecision( 6 ) << std::setw( 10 ) << autodiff_ms << " ms | " << std::setw( 15 )
    << 1e3 * autodiff_ms / timing_evaluations << " |\n"
    << "| TinyAD      | " << std::setw( 5 ) << points.size() << " | " << std::scientific << std::setprecision( 3 )
    << std::setw( 20 ) << worst_tinyad_error << " | " << std::setw( 20 ) << tinyad_rms_error << " | " << std::fixed
    << std::setprecision( 6 ) << std::setw( 10 ) << tinyad_ms << " ms | " << std::setw( 15 )
    << 1e3 * tinyad_ms / timing_evaluations << " |\n"
    ...
    << std::fixed << std::setprecision( 6 ) << std::setw( 10 ) << tinyad_scalar_ms << " ms | " << std::setw( 15 )
    << 1e3 * tinyad_scalar_ms / timing_evaluations << " |\n"
    << "+-------------+-------+----------------------+----------------------+--------------+-----------------+\n"
