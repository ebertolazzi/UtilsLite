...
namespace
{
  ...
  void
  clear_stream( std::ostringstream & s )
  {
    s.str( "" );
    s.clear();
  }
  ...
  bool
  contains( std::string const & str, std::string_view expected )
  {
    return str.find( std::string( expected ) ) != std::string::npos;
  }

  std::string
  strip_ansi( std::string_view text )
  {
    std::string plain;
  ...
  void
  expect_throws( std::string_view label, Fun && fun )
  {
    try
    {
      std::forward<Fun>( fun )();
  ...
  )
  {
    ...
    while ( true )
    {
      ...
      bool const reset_precedes_newline =
        newline >= reset.size() && got.compare( newline - reset.size(), reset.size(), reset ) == 0;
  ...
  test_style_methods()
  {
    ...
    expect_contains( C, out, "setMessageStyle", "message style deprecated\n", [&]() { C.message( "message style deprecated\n", 0 ); } );
    ...
    expect_contains( C, out, "setWarningStyle", "warning style deprecated\n", [&]() { C.warning( "warning style deprecated\n" ); } );
    ...
    expect_contains( C, out, "setErrorStyle", "error style deprecated\n", [&]() { C.error( "error style deprecated\n" ); } );
    ...
    expect_contains( C, out, "setFatalStyle", "fatal style deprecated\n", [&]() { C.fatal( "fatal style deprecated\n" ); } );
  ...
  test_legacy_message_methods()
  {
    ...
    expect_contains( C, out, "message old level", "message legacy level\n", [&]() { C.message( "message legacy level\n", 2 ); } );

    expect_contains( C, out, "semaphore old 0", "semaphore legacy 0\n", [&]() { C.semaphore( 0, "semaphore legacy 0\n" ); } );
    expect_contains( C, out, "semaphore old 1", "semaphore legacy 1\n", [&]() { C.semaphore( 1, "semaphore legacy 1\n" ); } );
    expect_contains( C, out, "semaphore old 2", "semaphore legacy 2\n", [&]() { C.semaphore( 2, "semaphore legacy 2\n" ); } );
    expect_contains( C, out, "semaphore old level", "semaphore legacy level\n", [&]() { C.semaphore( 2, "semaphore legacy level\n", 2 ); } );
    ...
    expect_contains( C, out, "colors old level", "colors legacy level\n", [&]() { C.colors( 3, "colors legacy level\n", 2 ); } );

    // Indices intentionally wrap: semaphore modulo 3, colors modulo 5.
    expect_contains( C, out, "semaphore wrap", "semaphore wrapped 3 -> 0\n", [&]() { C.semaphore( 3, "semaphore wrapped 3 -> 0\n" ); } );
    expect_contains( C, out, "colors wrap", "colors wrapped 5 -> 0\n", [&]() { C.colors( 5, "colors wrapped 5 -> 0\n" ); } );
    ...
    expect_contains( C, out, "black_reversed old", "black_reversed legacy\n", [&]() { C.black_reversed( "black_reversed legacy\n" ); } );
    expect_contains( C, out, "red_reversed old", "red_reversed legacy\n", [&]() { C.red_reversed( "red_reversed legacy\n" ); } );
    expect_contains( C, out, "green_reversed old", "green_reversed legacy\n", [&]() { C.green_reversed( "green_reversed legacy\n" ); } );
    expect_contains( C, out, "yellow_reversed old", "yellow_reversed legacy\n", [&]() { C.yellow_reversed( "yellow_reversed legacy\n" ); } );
    expect_contains( C, out, "blue_reversed old", "blue_reversed legacy\n", [&]() { C.blue_reversed( "blue_reversed legacy\n" ); } );
    expect_contains( C, out, "magenta_reversed old", "magenta_reversed legacy\n", [&]() { C.magenta_reversed( "magenta_reversed legacy\n" ); } );
    expect_contains( C, out, "cyan_reversed old", "cyan_reversed legacy\n", [&]() { C.cyan_reversed( "cyan_reversed legacy\n" ); } );
    expect_contains( C, out, "gray_reversed old", "gray_reversed legacy\n", [&]() { C.gray_reversed( "gray_reversed legacy\n" ); } );
  }
  ...
  test_formatted_message_methods()
  {
    ...
    for ( unsigned c = 0; c < 5; ++c )
    {
      std::string const expected = std::string( "colors formatted " ) + std::to_string( c ) + ": value " + std::to_string( 10 + c ) + "\n";
      expect_contains(
        C,
        out,
        "colors formatted",
    ...
    expect_contains( C, out, "warning formatted", "warning formatted: tol=1.23e-04 iter=5\n", [&]() { C.warning( "warning formatted: tol={:.2e} iter={}\n", 1.234e-4, 5 ); } );
    expect_contains( C, out, "error formatted", "error formatted: file.dat line=77\n", [&]() { C.error( "error formatted: {} line={}\n", "file.dat", 77 ); } );
    expect_contains( C, out, "fatal formatted", "fatal formatted: code=-9 reason=stop\n", [&]() { C.fatal( "fatal formatted: code={} reason={}\n", -9, "stop" ); } );

    expect_contains( C, out, "black formatted", "black formatted: A 1\n", [&]() { C.black( 0, "black formatted: {} {}\n", 'A', 1 ); } );
    expect_contains( C, out, "red formatted", "red formatted: B 2\n", [&]() { C.red( 0, "red formatted: {} {}\n", 'B', 2 ); } );
    expect_contains( C, out, "green formatted", "green formatted: C 3\n", [&]() { C.green( 0, "green formatted: {} {}\n", 'C', 3 ); } );
    expect_contains( C, out, "yellow formatted", "yellow formatted: D 4\n", [&]() { C.yellow( 0, "yellow formatted: {} {}\n", 'D', 4 ); } );
    expect_contains( C, out, "blue formatted", "blue formatted: E 5\n", [&]() { C.blue( 0, "blue formatted: {} {}\n", 'E', 5 ); } );
    expect_contains( C, out, "magenta formatted", "magenta formatted: F 6\n", [&]() { C.magenta( 0, "magenta formatted: {} {}\n", 'F', 6 ); } );
    expect_contains( C, out, "cyan formatted", "cyan formatted: G 7\n", [&]() { C.cyan( 0, "cyan formatted: {} {}\n", 'G', 7 ); } );
    expect_contains( C, out, "gray formatted", "gray formatted: H 8\n", [&]() { C.gray( 0, "gray formatted: {} {}\n", 'H', 8 ); } );

    expect_contains( C, out, "black_reversed formatted", "black_reversed formatted: A 11\n", [&]() { C.black_reversed( 0, "black_reversed formatted: {} {}\n", 'A', 11 ); } );
    expect_contains( C, out, "red_reversed formatted", "red_reversed formatted: B 12\n", [&]() { C.red_reversed( 0, "red_reversed formatted: {} {}\n", 'B', 12 ); } );
    expect_contains( C, out, "green_reversed formatted", "green_reversed formatted: C 13\n", [&]() { C.green_reversed( 0, "green_reversed formatted: {} {}\n", 'C', 13 ); } );
    expect_contains( C, out, "yellow_reversed formatted", "yellow_reversed formatted: D 14\n", [&]() { C.yellow_reversed( 0, "yellow_reversed formatted: {} {}\n", 'D', 14 ); } );
    expect_contains( C, out, "blue_reversed formatted", "blue_reversed formatted: E 15\n", [&]() { C.blue_reversed( 0, "blue_reversed formatted: {} {}\n", 'E', 15 ); } );
    expect_contains( C, out, "magenta_reversed formatted", "magenta_reversed formatted: F 16\n", [&]() { C.magenta_reversed( 0, "magenta_reversed formatted: {} {}\n", 'F', 16 ); } );
    expect_contains( C, out, "cyan_reversed formatted", "cyan_reversed formatted: G 17\n", [&]() { C.cyan_reversed( 0, "cyan_reversed formatted: {} {}\n", 'G', 17 ); } );
    expect_contains( C, out, "gray_reversed formatted", "gray_reversed formatted: H 18\n", [&]() { C.gray_reversed( 0, "gray_reversed formatted: {} {}\n", 'H', 18 ); } );
  }
  ...
  test_level_filtering()
  {
    ...
    expect_contains( C, out, "message visible at level", "visible message level 2\n", [&]() { C.message( "visible message level 2\n", 2 ); } );
    expect_empty( C, out, "message hidden at level", [&]() { C.message( "hidden message level 3\n", 3 ); } );
    expect_empty( C, out, "message default hidden at level 2", [&]() { C.message( "hidden default message level 4\n" ); } );

    expect_contains( C, out, "formatted message visible at level", "visible formatted 2\n", [&]() { C.message( 2, "visible formatted {}\n", 2 ); } );
    expect_empty( C, out, "formatted message hidden at level", [&]() { C.message( 3, "hidden formatted {}\n", 3 ); } );

    expect_contains( C, out, "color visible at level", "visible red level 2\n", [&]() { C.red( "visible red level 2\n", 2 ); } );
    expect_empty( C, out, "color hidden at level", [&]() { C.red( "hidden red level 3\n", 3 ); } );

    expect_contains( C, out, "formatted color visible at level", "visible red formatted 2\n", [&]() { C.red( 2, "visible red formatted {}\n", 2 ); } );
    expect_empty( C, out, "formatted color hidden at level", [&]() { C.red( 3, "hidden red formatted {}\n", 3 ); } );

    expect_contains( C, out, "semaphore visible at level", "visible semaphore level 2\n", [&]() { C.semaphore( 0, "visible semaphore level 2\n", 2 ); } );
    expect_empty( C, out, "semaphore hidden at level", [&]() { C.semaphore( 0, "hidden semaphore level 3\n", 3 ); } );

    expect_contains( C, out, "formatted semaphore visible at level", "visible semaphore formatted 2\n", [&]() { C.semaphore( 2, 0, "visible semaphore formatted {}\n", 2 ); } );
    expect_empty( C, out, "formatted semaphore hidden at level", [&]() { C.semaphore( 3, 0, "hidden semaphore formatted {}\n", 3 ); } );

    expect_contains( C, out, "colors visible at level", "visible colors level 2\n", [&]() { C.colors( 0, "visible colors level 2\n", 2 ); } );
    expect_empty( C, out, "colors hidden at level", [&]() { C.colors( 0, "hidden colors level 3\n", 3 ); } );

    expect_contains( C, out, "formatted colors visible at level", "visible colors formatted 2\n", [&]() { C.colors( 2, 0, "visible colors formatted {}\n", 2 ); } );
    expect_empty( C, out, "formatted colors hidden at level", [&]() { C.colors( 3, 0, "hidden colors formatted {}\n", 3 ); } );
    ...
    expect_contains( C, out, "formatted error visible at level 1", "visible error 1\n", [&]() { C.error( "visible error {}\n", 1 ); } );
    ...
    C.change_level( 0 );
    expect_contains( C, out, "fatal visible at level 0", "visible fatal at 0\n", [&]() { C.fatal( "visible fatal at {}\n", 0 ); } );

    C.change_level( -1 );
    expect_empty( C, out, "normal output hidden at level -1", [&]() { C.green( "hidden green\n", 0 ); } );
    expect_empty( C, out, "negative message level hidden at level -1", [&]() { C.message( "hidden message at -1\n", -1 ); } );
    expect_empty( C, out, "negative formatted level hidden at level -1", [&]() { C.red( -2, "hidden formatted at {}\n", -2 ); } );
