...
struct MacroExample
{
  ...
  template <typename T> static T f5( T const & a, T const & b, T const & c, T const & d, T const & e ) { return a * b + c * d + e * e; }
  template <typename T> static T f6( T const & a, T const & b, T const & c, T const & d, T const & e, T const & f ) { return a * b + c * d + e * f; }
...
int main()
{
  ...
  const bool ok =
    close_to( MacroExample::f1_D( x ), 2.0 * x ) &&
    close_to( MacroExample::f1_DD( x ), 2.0 ) &&
    close_to( MacroExample::f_D_1( x, y ), 2.0 * x + 3.0 * y ) &&
    close_to( MacroExample::f_D_2( x, y ), 3.0 * x + std::cos( y ) ) &&
    close_to( MacroExample::f_D_1_1( x, y ), 2.0 ) &&
    close_to( MacroExample::f_D_1_2( x, y ), 3.0 ) &&
