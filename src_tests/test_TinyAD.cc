...
bool close_to( double actual, double expected ) { return std::abs( actual - expected ) <= tolerance; }
...
int main()
{
  ...
  quadratic.template add_elements<1>( TinyAD::range( 1 ), []( auto & element )
  {
  ...
  passed += report( "value", value, 11.0 ); ++total;
  passed += report( "gradient[0]", gradient[0], 8.0 ); ++total;
  passed += report( "gradient[1]", gradient[1], 7.0 ); ++total;
  passed += report( "hessian(0,0)", hessian.coeff( 0, 0 ), 2.0 ); ++total;
  passed += report( "hessian(0,1)", hessian.coeff( 0, 1 ), 3.0 ); ++total;
  passed += report( "hessian(1,0)", hessian.coeff( 1, 0 ), 3.0 ); ++total;
  passed += report( "hessian(1,1)", hessian.coeff( 1, 1 ), 2.0 ); ++total;
  ...
  nonlinear.template add_elements<1>( TinyAD::range( 1 ), []( auto & element )
  {
    const auto x = element.variables( 0 );
    using std::exp;
    using std::sin;
  ...
  passed += report( "nonlinear value", nonlinear_value, point[0] * point[0] + 3.0 * point[0] * point[1] + point[1] * point[1] + sine_exp ); ++total;
  passed += report( "nonlinear gradient[0]", nonlinear_gradient[0], 2.0 * point[0] + 3.0 * point[1] + cosine_exp ); ++total;
  passed += report( "nonlinear gradient[1]", nonlinear_gradient[1], 3.0 * point[0] + 2.0 * point[1] + sine_exp ); ++total;
  passed += report( "nonlinear hessian(0,0)", nonlinear_hessian.coeff( 0, 0 ), 2.0 - sine_exp ); ++total;
  passed += report( "nonlinear hessian(0,1)", nonlinear_hessian.coeff( 0, 1 ), 3.0 + cosine_exp ); ++total;
  passed += report( "nonlinear hessian(1,1)", nonlinear_hessian.coeff( 1, 1 ), 2.0 + sine_exp ); ++total;

  banner( "4. Final result" );
  std::cout << "Checks passed: " << passed << '/' << total << '\n';
  if ( passed != total ) { std::cerr << "TinyAD integration test FAILED.\n"; return 1; }
